...
import OneAttendanceChart from "./OneAttendanceChart";

function OneSubjectAttendace() {
  const [data, setdata] = useState({
    courseData: { course_name: "", course_id: "", faculty_name: "" },
    attendance: [],
  });

  const { c_id, sec_id } = useParams();
  ...
  if (!data.courseData.course_name) {
      ...
      <div className="userdetails">
        ...
        >
          <div>
            ...
            <p>
              Faculty <ChevronRight size={16} fill="black" />{" "}
              <span>{data.courseData.faculty_name}</span>
            </p>
          </div>
        </div>
      </div>
...
export default OneSubjectAttendace;
